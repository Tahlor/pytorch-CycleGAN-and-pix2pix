...
class CycleGAN2Model(BaseModel):
    ...
    def forward(self):
        self.fake_B = self.mask(self.netG_A(self.real_A), self.real_A) # generate a fake B from an A
        self.rec_A = self.netG_B(self.fake_B)  # recreate A from a faked B (original A)

        self.fake_A = self.mask(self.netG_B(self.real_B), self.real_B)
