...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, save_result, step=None):
        ...
        if self.use_html and (save_result or not self.saved):  # save images to a html file
            ...
            text = "epoch{:03d}".format(epoch) if step is None else "step{:07d}".format(step)
            
            for label, image in visuals.items():
                image_numpy = util.tensor2im(image)
                img_path = os.path.join(self.img_dir, '{}_{}.png'.format(text, label))
            ...
            for n in range(epoch, 0, -1):
                ...
                for label, image_numpy in visuals.items():
                    image_numpy = util.tensor2im(image)
                    img_path = '{}_{}.png'.format(text, label)
                    ims.append(img_path)
                    txts.append(label)
